...
Please note that the paths can start or end at any node but all paths must follow direction from parent to
child (top to bottom).
"""
...
"""
Time complexity
The time complexity of the above algorithm is O(N^2) in the worst case, where
...
def find_current_count_ps(currentNode, S, count):
    if not currentNode:
        return 0
    ...
    return (
        count
        + find_current_count_ps(currentNode.left, S, count)
        + find_current_count_ps(currentNode.right, S, count)
        + find_current_count_ps(currentNode.left, S - currentNode.val, count)
        + find_current_count_ps(currentNode.right, S - currentNode.val, count)
    )
